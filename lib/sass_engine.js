...
var postprocessor = require('mq4-hover-shim/src/nodejs')
...
SassEngine.prototype.evaluate = function(context, locals) {
  ...
  try {
    ...
    var css = String(result.css || result);

    css = postprocessor.postprocessorFor({hoverSelectorPrefix: 'body.not-touch '}).process(css).css
